        ...
        <div className="flex items-center justify-between h-16 md:h-20">
            ...
            {navLinks.map((link) =>
              link.hasDropdown ? (
                ...
                >
                  <Link
                    href={link.href}
                    className="flex items-center gap-1 text-sm font-medium transition-colors py-2 text-[#3C2415] hover:text-[#8B6F47]"
                  >
                    {link.label}
                    <ChevronDown
                      size={14}
                  ...
                  {shopOpen && categories.length > 0 && (
                    <div className="absolute top-full left-0 mt-0 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 py-2.5 animate-in fade-in slide-in-from-top-2 duration-200 text-gray-800">
                      <Link
                        href="/shop"
                        className="block px-4 py-2.5 text-sm text-[#3C2415] hover:bg-[#F5F0EB] transition-colors font-semibold"
                      >
                        All Products
                      </Link>
                      ...
                      {categories.map((cat) => (
                        <Link
                          ...
                          className="block px-4 py-2 text-sm text-gray-600 hover:bg-[#F5F0EB] hover:text-[#3C2415] transition-colors"
                        >
                          {cat.name}
                        </Link>
                      ))}
                ...
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-sm font-medium transition-colors text-[#3C2415] hover:text-[#8B6F47]"
                >
                  {link.label}
                </Link>
              )
          ...
          <div className="flex items-center gap-3 md:gap-5">
            ...
            >
              ...
              {mounted && itemCount > 0 && (
                <span
                  className="absolute -top-2 -right-2 sm:hidden text-white text-xs w-5 h-5 rounded-full flex items-center justify-center font-medium shadow bg-[#8B6F47]"
                >
                  {itemCount}
                </span>
              )}
            ...
            {navLinks.map((link) => (
              <div key={link.href}>
                <Link
                  ...
                  className="block py-3 text-[#3C2415] font-medium border-b border-gray-50"
                >
                  {link.label}
                </Link>
                  ...
                  <div className="pl-4 pb-2 space-y-1">
                    {categories.map((cat) => (
                      <Link
                        ...
                        href={`/category/${cat.slug}`}
                        onClick={() => setMobileOpen(false)}
                        className="block py-1.5 text-sm text-gray-600 hover:text-[#3C2415]"
                      >
                        {cat.name}
                      </Link>
                    ))}
